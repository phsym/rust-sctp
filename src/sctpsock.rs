...
use std::net::{ToSocketAddrs, SocketAddr, SocketAddrV4, SocketAddrV6, Ipv4Addr, Ipv6Addr, Shutdown};
use std::mem::{size_of, MaybeUninit};

// import macros from lib
#[cfg(target_os="linux")]
use crate::{syscall, sctp_syscall};

// import sockaddr helpers from lib
#[cfg(target_os="linux")]
use mio_unix::{socket_addr, to_socket_addr};
...
mod win {
	...
	pub use ws2_32::{socket, closesocket};
	pub use winapi::{SOCKADDR as sockaddr, SOCKADDR_IN as sockaddr_in, sockaddr_in6, socklen_t, AF_INET, AF_INET6, SOCKET};
...
mod linux {
	...
	pub use libc::{sockaddr, sockaddr_in, sockaddr_in6, socklen_t, AF_INET, AF_INET6, socket, SHUT_RDWR, SHUT_RD, SHUT_WR, EINPROGRESS};
...
impl SctpAddrType {
	unsafe fn get(&self, sock: SOCKET, id: sctp_sys::sctp_assoc_t, ptr: *mut *mut sockaddr) -> libc::c_int {
		return match *self {
	...
	unsafe fn from_raw_ptr(addr: *const sockaddr, len: socklen_t) -> Result<SocketAddr> {
		...
		match (*addr).sa_family as libc::c_int {
			...
			AF_INET6 if len >= size_of::<sockaddr_in6>() as socklen_t => {
				...
				let socket_addr_v6 = SocketAddrV6::new(ip6_addr, u16::from_be(in6_addr.sin6_port), in6_addr.sin6_flowinfo, in6_addr.sin6_scope_id);
...
impl SctpSocket {
	...
	pub fn connectx<A: ToSocketAddrs>(&self, addresses: &[A]) -> Result<sctp_sys::sctp_assoc_t> {
		...
		let buf: *mut u8 = unsafe { libc::malloc((addresses.len() * size_of::<libc::sockaddr_storage>()) as libc::size_t) as *mut u8 };
		if buf.is_null() {
		...
		for address in addresses {
			let addrobj = SocketAddr::from_addr(&address)?;
			let (raw_addr, raw_addr_length) = socket_addr(&addrobj);
			unsafe { std::ptr::copy_nonoverlapping(raw_addr.as_ptr() as *mut u8, buf.offset(offset), raw_addr_length as usize) };
			offset += raw_addr_length as isize;
		}

		let mut assoc: sctp_sys::sctp_assoc_t = 0;

		match sctp_syscall!(sctp_connectx(self.0, buf as *mut sockaddr, addresses.len() as i32, &mut assoc))
		{
			Err(err) => { unsafe{libc::free(buf as *mut libc::c_void)};Err(err) },
	...
	pub fn bindx<A: ToSocketAddrs>(&self, addresses: &[A], op: BindOp) -> Result<()> {
		...
		let buf: *mut u8 = unsafe { libc::malloc((addresses.len() * size_of::<sockaddr_in6>()) as libc::size_t) as *mut u8 };
		if buf.is_null() {
			return Err(Error::new(ErrorKind::Other, "Out of memory"));
		}
		let mut offset = 0isize;
		...
		match sctp_syscall!(sctp_bindx(self.0, buf as *mut sockaddr, addresses.len() as i32, op.flag()))
		{
			Err(err) => { unsafe{libc::free(buf as *mut libc::c_void)};Err(err) },
			Ok(_) => { Ok(()) },
		}
	...
	pub fn accept(&self) -> Result<(SctpSocket, SocketAddr)> {
		...
		let stream = {
			syscall!(accept(self.0, addr_storage.as_mut_ptr() as *mut _, &mut addr_storage_length))
	...
	pub fn recvmsg(&self, msg: &mut [u8]) -> Result<(usize, u16, SocketAddr)> {
		...
		unsafe { to_socket_addr(addr_storage.as_ptr()) }.map(|addr| (recvlen as usize, info.sinfo_stream, addr))
	}
	...
	pub fn getsockopt<T>(&self, level: libc::c_int, optname: libc::c_int) -> Result<T> {
		...
		match syscall!(getsockopt(self.0, level, optname, &mut val as *mut T as *mut libc::c_void, &mut len))
		{
			Err(err) => Err(err),
			Ok(_) => Ok(val),
		}
	...
	pub fn sctp_opt_info<T>(&self, optname: libc::c_int, assoc: sctp_sys::sctp_assoc_t) -> Result<T> {
		...
		match sctp_syscall!(sctp_opt_info(self.0, assoc, optname, &mut val as *mut T as *mut libc::c_void, &mut len))
		{
			Err(err) => Err(err),
			Ok(_) => Ok(val),
		}
