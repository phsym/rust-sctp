...
mod win {
	...
	pub use ws2_32::{socket, connect, bind, listen, accept, recv, send, shutdown, setsockopt, closesocket};
...
impl RawSocketAddr for SocketAddr {
	...
	unsafe fn from_raw_ptr(addr: *const sockaddr, len: socklen_t) -> Result<SocketAddr> {
		...
		return match (*addr).sa_family as libc::c_int {
			AF_INET if len >= size_of::<sockaddr_in>() as socklen_t => Ok(SocketAddr::V4(transmute(*(addr as *const sockaddr_in)))),
			AF_INET6 if len >= size_of::<sockaddr_in6>() as socklen_t => Ok(SocketAddr::V6(transmute(*(addr as *const sockaddr_in6)))),
...
impl SctpSocket {
	...
	pub fn connectx<A: ToSocketAddrs>(&self, addresses: &[A]) -> Result<sctp_sys::sctp_assoc_t> {
		...
		unsafe {
			let buf: *mut u8 = libc::malloc((addresses.len() * size_of::<sockaddr_in6>()) as libc::size_t) as *mut u8;
			if buf.is_null() {
			...
			let ret = match sctp_sys::sctp_connectx(self.0, buf as *mut sockaddr, addresses.len() as i32, &mut assoc) {
				0 => Ok(assoc),
				_ => Err(Error::last_os_error()),
			};
	...
	pub fn bindx<A: ToSocketAddrs>(&self, addresses: &[A], op: BindOp) -> Result<()> {
		...
		unsafe {
			...
			let ret = match sctp_sys::sctp_bindx(self.0, buf as *mut sockaddr, addresses.len() as i32, op.flag()) {
				0 => Ok(()),
				_ => Err(Error::last_os_error())
			};
			libc::free(buf as *mut libc::c_void);
	...
	pub fn setsockopt<T>(&self, level: libc::c_int, optname: libc::c_int, optval: &T) -> Result<()> {
		unsafe {
			return match setsockopt(self.0, level, optname, transmute(optval), size_of::<T>() as socklen_t) {
				0 => Ok(()),
				_ => Err(Error::last_os_error())
			};
		}
